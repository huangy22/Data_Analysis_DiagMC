  ...
  MODULE my_vrbls
    ...
    integer            :: NCycle                          ! # no. of cycles of annealing
    ...
    integer, parameter  :: MxL = 12                            ! maximum lattice size 
    integer,parameter   :: MaxKink=128
    ...
    integer, parameter :: NObs_b = 16                         ! #basic observables
    ...
    double precision       :: JJ4                               ! common variable 
    double precision       :: Q
    double precision       :: QQ
    double precision       :: ExternalField
    integer                :: IsAF                             ! 0: ferro; 1: AF
  ...
  PROGRAM main
    use my_vrbls
    implicit none
    integer :: icycle, itoss,isamp,iblck,blck, i,k,Site,Low
    ...
    print *, 'Lattice, L, beta, J,Q,Hz,IsAF, Ntoss, Nsamp, Nswee, NSave, NCycle,Seed, NBlck, IsLoad, Outputfile, files:'
    read  *,  LatticeName,L(:),beta,JJ,Q,ExternalField, IsAF, Ntoss,Nsamp,Nswee,NSave, &
	    &  NCycle,Seed,NBlck, IsLoad,filename,statfile, midfile
	...
	nnb = 2*Dim
	totnb = nnb
    elseif(LatticeName=='3DCheckerboard') then
	NSub = 2
	nnb = 2*Dim
	totnb = nnb
    ...
    if(LatticeName=='3DCheckerboard' .and. Dim/=3) then
	print *,"3DCheckerboard on ", Dim, "dimension does not exist!"
	stop
    endif
  ...
  SUBROUTINE carlo
	...
	46 format(' Simulation is on L=',i6,'*',i6,'*',i6,2x,a15,1x,'lattice.')
    endif
    ...
    allocate(Obs(1:NObs, 1:NCycle*NBlck));       Obs = 0.d0
  ...
  SUBROUTINE def_lattice
    ...
    else if(LatticeName=='3DCheckerboard') then
      !RealVectors
      LatticeVector(1,:) = (/1.0, 0.0, 0.0/)
      LatticeVector(2,:) = (/0.0, 1.0, 0.0/)
      LatticeVector(3,:) = (/0.0, 0.0, 1.0/)
	...
	site(i) = i
      ...
      !directions
      Back(1, 1) = 4; Back(4, 2) = 1;
      Back(2, 1) = 5; Back(5, 2) = 2;
      Back(3, 1) = 6; Back(6, 2) = 3;
      Back(4, 1) = 1; Back(1, 2) = 4;
      Back(5, 1) = 2; Back(2, 2) = 5;
      Back(6, 1) = 3; Back(3, 2) = 6;
	...
	do iy=1,L(2)
	  do ix=1,L(1)
	    ...
	    !!!!!NEAREST NEIGHBOR!!!!!!!!!!!!!!!!!!!!!!
	    Ngs(1, site(1)) = site(2)
	    Ngs(back(1, 1),site(2)) = site(1)

    	    call GetSite(itmp, (/ix, iym, iz/), 2)
	    Ngs(2, site(1))  = itmp
	    Ngs(back(2, 1), itmp) = site(1)

    	    call GetSite(itmp, (/ixm, iym, izp/), 2)
	    Ngs(3, site(1))  = itmp
	    Ngs(back(3, 1), itmp) = site(1)

    	    call GetSite(itmp, (/ixm, iym, iz/), 2)
	    Ngs(4, site(1)) =  itmp
	    Ngs(back(4, 1), itmp) = site(1)

    	    call GetSite(itmp, (/ixm, iy, iz/), 2)
	    Ngs(5, site(1)) =  itmp
	    Ngs(back(5, 1), itmp) = site(1)

    	    call GetSite(itmp, (/ix, iy, izm/), 2)
	    Ngs(6, site(1)) =  itmp
	    Ngs(back(6, 1), itmp) = site(1)
	...
	enddo
      ...
      dr(:,:,:) = 0
      dr(1,1, 1) = 1;   dr(1,1, 2) =  1;   dr(1,1, 3) =  0
      dr(4,1, 1) =-1;   dr(4,1, 2) = -1;   dr(4,1, 3) =  0 
      dr(2,1, 1) = 1;   dr(2,1, 2) = -1;   dr(2,1, 3) =  0
      dr(5,1, 1) =-1;   dr(5,1, 2) =  1;   dr(5,1, 3) =  0 
      dr(3,1, 1) =-1;   dr(3,1, 2) = -1;   dr(3,1, 3) =  2    
      dr(6,1, 1) = 1;   dr(6,1, 2) =  1;   dr(6,1, 3) = -2     

      dr(1,2, 1) = 1;   dr(1,2, 2) =  1;   dr(1,2, 3) =  0
      dr(4,2, 1) =-1;   dr(4,2, 2) = -1;   dr(4,2, 3) =  0 
      dr(2,2, 1) = 1;   dr(2,2, 2) = -1;   dr(2,2, 3) =  0
      dr(5,2, 1) =-1;   dr(5,2, 2) =  1;   dr(5,2, 3) =  0 
      dr(3,2, 1) =-1;   dr(3,2, 2) = -1;   dr(3,2, 3) =  2    
      dr(6,2, 1) = 1;   dr(6,2, 2) =  1;   dr(6,2, 3) = -2     
    endif
    call PrintLattice
  ...
  SUBROUTINE GetVector(Site, Vector, Sub)
    ...
    else if(LatticeName=='Pyrochlore' .or. LatticeName=='3DCheckerboard') then
	Sub = mod(Site, NSub)
	if(Sub==0) Sub = NSub
	tmp = (Site-1)/NSub
  ...
  SUBROUTINE PrintLattice()
	...
	DO j=1, Vol 
		...
		do i = 1, Dim
		    ...
		    if(deltar(i)/=0) then
			print *, j, nb, i, deltar(i)
			print *, RealVector(Ngs(nb, j),:)-RealVector(j,:)
			!stop
		    endif
		enddo
	    ENDDO
  ...
  SUBROUTINE monte
	    ...
	    do k = 1, Dim
		if(LatticeName=="Pyrochlore" .or. LatticeName=='3DCheckerboard') then
    ...
    SUBROUTINE adjust_JJ(newJ)
        implicit none
        integer :: i,j,k
	double precision :: newJ
	Coupling(1:nnb,1:Vol) = newJ
	EnergyCheck = potential_energy()
	call check_config()
	return
    END SUBROUTINE adjust_JJ
    ...
    SUBROUTINE create_worm(flag)
        ...
        !!!!!!!!!!!!!!!!!!    fancy way   !!!!!!!!!!!!!!!!!!!!!!!!!!
        dTime=Tmax-itime
        R=(1-exp(-JJ4*dTime))/JJ4
        tau=-1.0/JJ4*log(1-rn()*R*JJ4)  !positive
        mtime=tau+itime
        Factor=Comp*SegmentNum(vertex)*R*exp(JJ4*tau)*(Tmax-Tmin)
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    SUBROUTINE annihilate_worm(flag)
        ...
        double precision :: dTime, R, tau, itime, mtime
        integer :: EndSite
        logical :: flag
        ...
        dTime=Tmax-itime
        R=(1-exp(-JJ4*dTime))/JJ4
        tau=mtime-itime  !positive
        Factor=1.0/Comp/(SegmentNum(IraVertex)-2)/R*exp(-JJ4*tau)/(Tmax-Tmin)
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    SUBROUTINE move_worm()
        ...
        double precision :: total_E
        double precision :: R, tau, R_back, delta_Time2, Factor
        indicator=1
        ...
        if(Site1==1) then
            ...
            !!!!!!!!!!!!!!!   Fancy way     !!!!!!!!!!!!!!!!!!!!!!
            if(rn()<0.5) then
              delta_Time=Time2-IraTime
              R=(1-exp(-JJ4*delta_Time))/JJ4
              tau=-1.0/JJ4*log(1-rn()*R*JJ4)  !positive
              TarTime=IraTime+tau
              if(TarTime==IraTime .or. TarTime==Time2) return
              R_back=(1-exp(-JJ4*(TarTime-Time1+beta)))/JJ4
            else
              delta_Time=IraTime-Time1+beta
              R=(1-exp(-JJ4*delta_Time))/JJ4
              tau=-1.0/JJ4*log(1-rn()*R*JJ4)  !positive
              TarTime=IraTime-tau
              R_back=(1-exp(-JJ4*(Time2-TarTime)))/JJ4
              ...
              if(TarTime==IraTime .or. TarTime==Time1) return
            endif
            Factor=R/R_back
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            !!!!!!!!!!!!!!!   Fancy way     !!!!!!!!!!!!!!!!!!!!!!
            if(rn()<0.5) then
              delta_Time=Time2+beta-IraTime
              R=(1-exp(-JJ4*delta_Time))/JJ4
              tau=-1.0/JJ4*log(1-rn()*R*JJ4)  !positive
              TarTime=IraTime+tau
              R_back=(1-exp(-JJ4*(TarTime-Time1)))/JJ4
              if(TarTime>beta) then 
                indicator=3
                tarTime=tarTime-beta
                total_E=single_delta_E(IraVertex,IraSite,IraTime,beta)+single_delta_E(IraVertex,1,0.d0,tarTime)
              ...
              delta_Time=IraTime-Time1
              R=(1-exp(-JJ4*delta_Time))/JJ4
              tau=-1.0/JJ4*log(1-rn()*R*JJ4)  !positive
              TarTime=IraTime-tau
              R_back=(1-exp(-JJ4*(Time2+beta-TarTime)))/JJ4
              if(TarTime==IraTime .or. TarTime==Time1) return
            endif
            Factor=R/R_back
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	!if(Number>=10000) then
	    !write(20, *) delta_Time, exp(-total_E)
	    !if(total_E==0.d0) then
		!print *, indicator, IraVertex, IraSite, TarTime, IraTime
	    !endif
	    !!print *, abs(tarTime-IraTime), exp(-total_E)
	!endif
    ...
    SUBROUTINE create_kink()
        ...
        double precision    :: delta_Time, tau, R
        ...
        if(IsPast==1) then
            ...
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            !!!!!!!!!!!!  fancy way  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            delta_Time=IraTime-TargetTime
            R=(1-exp(-JJ4*delta_Time))/JJ4
            tau=-1.0/JJ4*log(1-rn()*R*JJ4)  !positive
            NewKinkTime=IraTime-tau
            Factor=real(nnb)*R*exp(JJ4*tau)*weight
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if(NewKinkTime==IraTime .or. NewKinkTime==TargetTime)return
    ...
    SUBROUTINE delete_kink()
        ...
        double precision    :: R, delta_Time,tau
        IsPast=int(rn()*2)
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !!!!!!!!!!!!  fancy way  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        delta_Time=abs(TargetTime-IraTime)
        tau=abs(OldKinkTime-IraTime)
        R=(1-exp(-JJ4*delta_Time))/JJ4
        Factor=1.0/real(nnb)/R*exp(-JJ4*tau)/weight
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	if(IsAF==1) then
	    ...
	    !!!!!Ferromagnetic
	    potential_E=-ExternalField*Energy
	endif
  ...
  SUBROUTINE measure
    ...
    Quan( 4)= Quan(3)**2.d0            !! Energy^2/Vol^2
    ...
    Quan( 9)= totM()**2              !! winding on tau==Magnetization
    Quan(10)= Quan( 9)**2              !! M^4
    Quan(11)= Quan( 9)*Vol*Beta        !! Spatial Susceptibility 
    Quan(12)= SM()**2                  !! Staggerd Magnetization 
    Quan(13)= Quan(12)**2              !! staggered M^4
    Quan(14)= Quan(12)*Vol*Beta        !! Spatial Susceptibility 
    Quan(15)= W0
    Quan(16)= W1
  ...
  SUBROUTINE winding_number()
	  ...
	  do while(Site/=2)
		  ...
		  if(dr(NeighVertexNum(Vertex,Site),Sub(Vertex),i)==2) then
		      dist(i)=dist(i)-SegmentState(Vertex,Site)*2
		  endif
	      enddo
	      Site=NextSite(Vertex,Site)
	  enddo
  ...
  double precision FUNCTION totM()
    implicit none
    double precision :: M, Sgn
    integer :: Vertex,R(1:Dim), sub,  i
    M=0.0
    do Vertex = 1, Vol
	M=M+0.5d0*SegmentState(Vertex,1)
    enddo
    totM=M/Vol
  end FUNCTION totM
  ...
  double precision FUNCTION SM()
    ...
    M=0.0
    do Vertex = 1, Vol
	M=M+0.5d0*SegmentState(Vertex,1)*get_Sign(Vertex)
    enddo
    SM=M/Vol
  end FUNCTION SM
  ...
  SUBROUTINE write2file 
    ...
    write(8,50) ident, L(1), beta, JJ*4.0, J3*4.0, Q*2.0, Seed, TotSamp
    50 format(a8,i6,4f12.6,i12,i8,i8)
  ...
  SUBROUTINE midwrite2file(iblck)
    ...
    !enddo
    write(8,*)
    write(8,50) ident, L(1), beta, JJ*4.0, J3*4.0, Q*2.0, Seed, NSamp, iblck
    50 format(a8,i6,4f12.6,i12,i8,i8)
    do j = 1, NObs_b
  ...
  SUBROUTINE stat_alan 
    ...
    nor  = 1.d0/(NCycle*NBlck*1.d0)

    ! -- calculate average -------------------------------------------
    do j = 1, NObs_b
      Ave(j) = nor*Sum(Obs(j,1:NCycle*NBlck))
    enddo
    ...
    Coarsen: do
	  ...
	  devn   = Obs(j,k)-Ave(j)
        ...
        Dev(j)   = dsqrt(Dev(j)/(NCycle*NBlck-1.d0))
        if(dabs(Cor(j))>tol) prt = .false.
      ENDDO 


      IF(prt)                         EXIT Coarsen 
      IF(NBlck*NCycle<64)    THEN
    ...
    do j = 1+NObs_b, NObs
      ...
      Dev(j)   = Dev(j)*nor;        Cor(j) = Cor(j)*nor
      IF(Dev(j)>eps)                Cor(j) = Cor(j)/Dev(j)
      Dev(j)   = dsqrt(Dev(j)/(NBlck*NCycle-1.d0))
    enddo
    return
